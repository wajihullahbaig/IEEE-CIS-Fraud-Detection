...
for productCD in train_c.ProductCD.unique().tolist():   
    train = train_c[train_c.ProductCD == productCD]
    test_idx = test_c[test_c.ProductCD == productCD].index.values
    test = test_c[test_c.ProductCD == productCD]
    print("Product Selection:", productCD)
    ...
    FE.expand_id31_and_DeviceInfo2(train,test)    
    ...
    train["ProductCD_fractioned"] = productCD + train["TransactionAmt_fractional"].astype(str) 
    test["ProductCD_fractioned"] = productCD + test["TransactionAmt_fractional"].astype(str)
    ...
    for col in tqdm(time_features):
        ...
        train[col+'_'+"ProductCD"] = train[col].astype(str) + productCD
        test[col+'_'+"ProductCD"] = test[col].astype(str) + productCD
    ...
    print("Selection len:",len(all_cols))
    ...
    if use_V_log_features:
        ...
        for col in tqdm(V_cols):
            ...
            train[col+'_ProductCD'] = train[col].astype(str) + productCD
            test[col+'_ProductCD'] = test[col].astype(str) + productCD
    ...
    plt.plot(fpr,tpr,label=productCD+" auc="+str(v_pred))
    plt.legend(loc=4)
    
    plt.figure(1)
    fpr, tpr, thresh = roc_curve(valid_y, ho_proba)
    plt.plot(fpr,tpr,label=productCD+" auc="+str(v_proba))
    plt.legend(loc=4)
    
    preds_proba[test_idx] = clf.predict_proba(test[all_cols], num_iteration=clf.best_iteration_)[:, 1]
    preds[test_idx] = clf.predict(test[all_cols], num_iteration=clf.best_iteration_)
    train = train_c.copy()
    test = test_c.copy()
    del clf
